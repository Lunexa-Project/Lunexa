...
gsigs = 'https://github.com/lunexa-project/gitian.sigs.git'
...
def setup():
    ...
    os.chdir('inputs')
    if os.path.isdir('lunexa'):
        # Remove the potentially stale lunexa dir. Otherwise you might face submodule mismatches.
        subprocess.check_call(['rm', 'lunexa', '-fR'])
    subprocess.check_call(['git', 'clone', args.url, 'lunexa'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/lunexa/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'lunexa='+args.commit, '--url', 'lunexa='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/lunexa-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/lunexa/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/lunexa/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/lunexa-project/lunexa', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/lunexa', exist_ok=True)
    os.chdir('builder/inputs/lunexa')
    if args.pull:
