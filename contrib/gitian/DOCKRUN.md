...
*Setup instructions for a Gitian build of Lunexa using Docker.*
...
You'll also need to clone the Lunexa repository and navigate to the `contrib/gitian` directory:

```bash
git clone https://github.com/lunexa-project/lunexa.git
cd lunexa/contrib/gitian
```
...
* a fork of the [gitian.sigs](https://github.com/lunexa-project/gitian.sigs/) repo on your GitHub account.
...
You can also look in the [gitian.sigs](https://github.com/lunexa-project/gitian.sigs/) repo and / or [do not currently have one]() to see if others got the same checksum for the same version tag.  If there is ever a mismatch -- **STOP! Something is wrong**.  Contact others on IRC / GitHub to figure out what is going on.
