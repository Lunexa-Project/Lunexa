...
# a main offline lunexad, does most of the tests
# a restricted RPC lunexad setup with RPC payment
# two local online lunexads connected to each other
N_LUNEXADS = 4

# 4 wallets connected to the main offline lunexad
# 1 wallet connected to the first local online lunexad
# 1 offline wallet
N_WALLETS = 6
...
lunexad_base = [builddir + "/bin/lunexad", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--no-igd", "--p2p-bind-port", "lunexad_p2p_port", "--rpc-bind-port", "lunexad_rpc_port", "--zmq-rpc-bind-port", "lunexad_zmq_port", "--zmq-pub", "lunexad_zmq_pub", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "lunexad_data_dir", "--log-level", "1"]
lunexad_extra = [
  ["--offline"],
...
wallet_base = [builddir + "/bin/lunexa-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--disable-rpc-login", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1", "--allow-mismatched-daemon-version"]
wallet_extra = [
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
...
for i in range(N_LUNEXADS):
  command_lines.append([str(18180+i) if x == "lunexad_rpc_port" else str(18280+i) if x == "lunexad_p2p_port" else str(18380+i) if x == "lunexad_zmq_port" else "tcp://127.0.0.1:" + str(18480+i) if x == "lunexad_zmq_pub" else builddir + "/functional-tests-directory/lunexad" + str(i) if x == "lunexad_data_dir" else x for x in lunexad_base])
  if i < len(lunexad_extra):
    command_lines[-1] += lunexad_extra[i]
  outputs.append(open(FUNCTIONAL_TESTS_DIRECTORY + '/lunexad' + str(i) + '.log', 'a+'))
