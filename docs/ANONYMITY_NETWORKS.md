# Anonymity Networks with Lunexa

Currently only Tor and I2P have been integrated into Lunexa. The usage of
...
is made or until `lunexad` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `lunexa-wallet-cli` and
`lunexa-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--tx-proxy` which tells `lunexad` the ip/port of a socks proxy provided by a
...
which tells `lunexad` that ".onion" P2P addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `lunexad` the inbound address, network
...
which tells `lunexad` that a max of 25 inbound Tor connections are being
received at address "5tymba6faziy36md5ffy42vatbjzlye4vyr3gyz6lcvdfximnvwpmwqd.onion:28083" and forwarded to `lunexad`
localhost port 28083, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p:5000" and
forwarded to `lunexad` localhost port 30000.
...
This will store key information in `/var/lib/tor/data/lunexa` and will forward
"Tor port" 28083 to port 28083 of ip 127.0.0.1. The file
`/usr/lib/tor/data/lunexa/hostname` will contain the ".onion" address for use
...
system clock is fairly accurate - many peers on the Lunexa network should have
similar timestamps.
...
### Intermittent Lunexa Syncing

If a user only runs `lunexad` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `lunexad` as often as possible to conceal when transactions are being sent.
...
`lunexad` currently selects two outgoing connections every 5 minutes for
...
`lunexad` will have rotated its selected outgoing connections several times in
