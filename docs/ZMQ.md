# The Current/Future Status of ZMQ in Lunexa

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. Lunexa
...
matching supported by both Lunexa and ZMQ. The `format`, `context` and `event`
...
The Lunexa daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The Lunexa daemon also ensures that
...
The Lunexa daemon will send a `txpool_add` pub exactly once for each
